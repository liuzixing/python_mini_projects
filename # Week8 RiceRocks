...
def group_group_collide(A,B):
    ...
    for a in A:
        for b in B:
            if a.collide(b):
                remove_A.add(a)
                explosion_group.add(Sprite(a.pos,[0,0], 0, 0 , explosion_image,explosion_info,explosion_sound))
                remove_B.add(b)
    if len(remove_A) > 0:
        A.difference_update(remove_A)
        B.difference_update(remove_B)
    return len(remove_A)
...
class Ship:
    ...
    def shoot(self):
        ...
        missile_sprite = Sprite(missile_pos, missile_vel, self.angle, 0, missile_image, missile_info, missile_sound)
...
def draw(canvas):
    global time, started, rock_group, lives, missile_group, score, started
...
def rock_spawner():
    ...
    rock_vel = [random.random() * .6 * (score / 10 + 1)- .3, random.random() * .6 * (score / 10 + 1) - .3]
    rock_avel = random.random() * .2 - .1
    rock_sprite = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    colli = rock_sprite.collide(my_ship)
    if len(rock_group) < 12 and (not colli):
